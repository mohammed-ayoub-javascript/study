...
import { Book, Loader2, Trash2, Edit, BookOpen, ChevronRight, Check, X, PlusCircle } from "lucide-react"
...
const ShowDetailsOfSubject = () => {
  ...
  const handleUpdateTitle = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      await axios.put(`${API}/api/subjects/${id}`, 
        { Title: newTitle },
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-8 z-10 relative text-white">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6">
        <div className="space-y-1 flex-1">
          <button onClick={() => router.push("/session/subjects")} className="text-gray-400 hover:text-white flex items-center gap-1 text-sm mb-2 transition-colors">
          ...
          {isEditing ? (
            <div className="flex items-center gap-2 max-w-md">
              ...
              <Button size="icon" variant="ghost" onClick={() => { setIsEditing(false); setNewTitle(subject?.Title || ""); }}>
        ...
        <div className="flex gap-3">
          {!isEditing && (
            <Button variant="outline" onClick={() => setIsEditing(true)} className="gap-2 border-white/10 hover:bg-white/5">
      ...
      <div className="space-y-6">
        ...
        {books.length === 0 ? (
          <div className="bg-white/5 border border-dashed border-white/10 rounded-2xl p-16 text-center">
            ...
            <p className="text-gray-400 mt-1 max-w-xs mx-auto">ابدأ بإضافة أول مراجعك الدراسية لهذه المادة لتنظيم مذاكرتك.</p>
            <Button 
              onClick={() => router.push(`/session/book/new?subject=${subject?.ID}`)} 
              variant="default" 
            ...
            {books.map((book) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <h3 className="font-bold text-lg mb-2 group-hover:text-primary transition-colors">{book.Title}</h3>
