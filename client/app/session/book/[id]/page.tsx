 ...
 const fetchData = async () => {
  ...
  try {
    ...
    if (bookRes.data.SubjectId) {
      const subjectRes = await axios.get(`${API}/api/subjects/${bookRes.data.SubjectId}`, { headers });
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black text-white antialiased">
      <div className="absolute top-0 left-0 w-full h-full z-0 overflow-hidden pointer-events-none opacity-40">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 min-w-full min-h-full w-auto h-auto">
          <YouTube 
            videoId={videoId} 
            opts={{
              playerVars: { autoplay: 1, controls: 0, rel: 0, mute: audio ? 1 : 0, loop: 1, playlist: videoId }
            }} 
      ...
      {isBreak ? (
        <div className="relative z-30 flex flex-col items-center justify-center h-screen w-full animate-in fade-in zoom-in duration-700">
           <div className="text-center p-12 rounded-[3rem] bg-white/5 backdrop-blur-2xl border border-white/10 shadow-2xl">
            ...
            <button onClick={() => setIsBreak(false)} className="mt-12 px-10 py-4 bg-white/10 rounded-2xl border border-white/10 transition-all">إنهاء الراحة</button>
          </div>
        </div>
      ) : (
        <div className="relative z-20 flex flex-col items-center p-6 md:p-12 h-screen overflow-y-auto custom-scrollbar">
          ...
          <div className="fixed top-6 left-6 right-6 flex justify-between items-center z-50">
            <button onClick={() => router.back()} className="p-3 bg-white/5 backdrop-blur-xl rounded-full border border-white/10 hover:bg-white/10 transition-all">
              <ArrowLeft />
            </button>
            <div className="flex gap-3">
               <div className="hidden md:flex flex-col items-end px-4">
                  <span className="text-orange-500 font-bold text-xs uppercase tracking-widest">{subject?.Title || "بدون مادة"}</span>
                  <span className="text-white text-lg font-bold">{book?.Title}</span>
               </div>
               <button onClick={() => setAudio(!audio)} className="p-3 bg-white/5 backdrop-blur-xl rounded-full border border-white/10 hover:bg-white/10 transition-all">
          ...
          <header className="mb-12 text-center mt-12">
            <h1 className="text-2xl font-light tracking-[0.3em] text-gray-400 uppercase">Focus Session</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 w-full max-w-6xl">
            <div className="space-y-6">
              <div className="bg-white/5 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/10 shadow-2xl text-center">
                <div className="text-[7rem] font-thin tracking-tighter mb-8 tabular-nums">{formatTime(timeLeft)}</div>
                <div className="flex gap-4 justify-center">
                  ...
                  <button onClick={() => {setIsActive(false); setTimeLeft(25 * 60);}} className="px-8 py-5 bg-white/5 hover:bg-white/10 rounded-2xl border border-white/10 transition-all">إعادة</button>
                </div>
              </div>
              ...
              <div className="bg-black/40 backdrop-blur-md p-8 rounded-[2rem] border border-white/10">
                <h3 className="text-lg font-semibold mb-5 flex items-center gap-3">
                  <span className="p-2 bg-orange-500/20 rounded-lg text-orange-500">✏️</span> تدوين سريع لـ {book?.Title}
                </h3>
                <input 
                  type="text" 
                ...
                />
                <button onClick={saveNote} className="w-full bg-orange-500 text-white py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-orange-600 transition-all">حفظ في السجل</button>
              </div>
            </div>
            ...
            <div className="flex flex-col h-full max-h-[700px]">
              ...
              <div className="space-y-4 overflow-y-auto pr-4 custom-scrollbar">
                {notes?.length === 0 ? (
                  <div className="text-center py-20 bg-white/5 rounded-3xl border border-dashed border-white/10 text-gray-500">لا توجد ملاحظات لهذا الكتاب بعد</div>
                ) : (
                  notes?.map((item) => (
                    <div key={item.ID} className="bg-white/5 hover:bg-white/10 p-6 rounded-2xl border border-white/5 transition-all">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex flex-col">
                          <span className="text-[10px] text-orange-500 font-bold uppercase">Page</span>
                          <span className="text-xl font-mono">#{item.page_number || '--'}</span>
                        </div>
                        <span className="text-[10px] text-gray-500">{new Date(item.created_at).toLocaleDateString('ar-DZ')}</span>
                      </div>
      ...
      <style jsx global>{`
        .youtube-container iframe { width: 100vw; height: 100vh; transform: scale(1.8); }
